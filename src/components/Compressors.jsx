...
function Compressor({compressor_index}) {
	...
	async function init_or_update_chart() {
		...
		if (!charts[0]) {
			...
			charts[0].data = build_chart_data(0)
			charts[0].update()
		}
...
export function Compressors() {
	...
	return (
		<>
			...
			{data !== null && data.logs[0].compressors.map((compressor,index) => {
				return (
					<React.Fragment key={index}>
						<Compressor compressor_index={index}/>
					</React.Fragment>
