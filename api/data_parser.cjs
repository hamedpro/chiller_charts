...
function parse_data_part(string) {
	...
	for (let i = 0; i < parts_count; i++) {
		...
		//uncomment this comment below to overwrite an existing field when there is a duplicate for it
		/* if (Object.keys(result).includes(key)) {
            console.log(`duplicate field was detected : field key is same : field key = "${key}"`)
        }
        result[key] = Number(val.join('')) */

		result[
			`${key}*${
				Object.keys(result).filter((field) => field.split("*")[0] === key).length + 1
			}`
		] = Number(val.join(""));
	}
	return result;
...
function process_line(line) {
	...
	errors.forEach((error) => {
		...
		if (Number(splited_error[1]) !== 0) {
			tmp[splited_error[0].replace("Er", "")] = Number(splited_error[1]);
		}
	});
	errors = tmp;
	return {
...
function process_data(file_path, lines_limit = undefined) {
	...
	var lines = processed_data
		.slice(1, processed_data.length)
		.map((raw_line) => process_line(raw_line));
...
function gen_processed_file(file_path, lines_limit = undefined) {
	fs.writeFileSync(
		"processed_data.json",
		JSON.stringify(process_data(file_path, lines_limit), null, 4)
	);
}
